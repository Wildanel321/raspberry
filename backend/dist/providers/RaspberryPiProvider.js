...
exports.RaspberryPiProvider = void 0;
...
class RaspberryPiProvider {
    ...
    async getSystemInfo() {
        ...
        try {
            if (fs_1.default.existsSync('/proc/device-tree/model')) {
                model = fs_1.default.readFileSync('/proc/device-tree/model', 'utf8').replace(/\0/g, '');
            }
        }
        catch { }
    ...
    async getCPUStatus() {
        ...
        try {
            if (fs_1.default.existsSync('/sys/class/thermal/thermal_zone0/temp')) {
                const rawTemp = fs_1.default.readFileSync('/sys/class/thermal/thermal_zone0/temp', 'utf8');
        ...
        let freq = 0;
        try {
            if (fs_1.default.existsSync('/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq')) {
                const rawFreq = fs_1.default.readFileSync('/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq', 'utf8');
    ...
    async getNetworkStatus() {
        const interfaces = [];
        const osInterfaces = os_1.default.networkInterfaces();
        let totalRx = 0;
        let totalTx = 0;
        // Parse rx/tx bytes from /proc/net/dev
        try {
            const netdev = fs_1.default.readFileSync('/proc/net/dev', 'utf8');
        ...
        const internetConnected = await new Promise((resolve) => {
    ...
    async getGPIOStatus() {
        ...
        for (const pin of this.gpioPins) {
            if (pin.type === 'gpio' && pin.gpioPin !== null) {
                ...
                if (fs_1.default.existsSync(pinPath)) {
                    try {
                        const dir = fs_1.default.readFileSync(path_1.default.join(pinPath, 'direction'), 'utf8').trim();
                        const val = fs_1.default.readFileSync(path_1.default.join(pinPath, 'value'), 'utf8').trim();
    ...
    async toggleGPIO(pin, value) {
        ...
        if (!pinObj || pinObj.gpioPin === null || pinObj.type !== 'gpio')
            return false;
        ...
        try {
            if (!fs_1.default.existsSync(pinPath)) {
                // Export pin
                fs_1.default.writeFileSync('/sys/class/gpio/export', gp.toString());
            }
            // Ensure it is output mode
            fs_1.default.writeFileSync(path_1.default.join(pinPath, 'direction'), 'out');
            fs_1.default.writeFileSync(path_1.default.join(pinPath, 'value'), value.toString());
            pinObj.mode = 'out';
            pinObj.value = value;
            return true;
        }
    ...
    async setGPIOMode(pin, mode) {
        ...
        try {
            ...
            fs_1.default.writeFileSync(path_1.default.join(pinPath, 'direction'), mode);
            pinObj.mode = mode;
            return true;
        }
        catch (e) {
    ...
    async getDisplayStatus() {
        ...
        try {
            ...
            if (fs_1.default.existsSync(drmPath)) {
                const dirs = fs_1.default.readdirSync(drmPath);
                for (const dir of dirs) {
                    if (dir.includes('HDMI-A') || dir.includes('card0-DP')) {
                        const statusPath = path_1.default.join(drmPath, dir, 'status');
                        if (fs_1.default.existsSync(statusPath)) {
                            const status = fs_1.default.readFileSync(statusPath, 'utf8').trim();
                            if (status === 'connected') {
                                connected = true;
                                break;
                            }
    ...
    async getSecurityStatus() {
        ...
        try {
            if (fs_1.default.existsSync('/etc/ssh/sshd_config')) {
                const config = fs_1.default.readFileSync('/etc/ssh/sshd_config', 'utf8');
    ...
    async runPackageAction(action, pkgName) {
        ...
        else if (action === 'install' && pkgName)
            cmd = `sudo apt-get install -y ${pkgName}`;
        else if (action === 'remove' && pkgName)
            cmd = `sudo apt-get remove -y ${pkgName}`;
        else
            return 'Invalid package parameters';
    ...
    async getLogs(type, query, limit = 100) {
        ...
        if (type === 'journal') {
            ...
            if (fs_1.default.existsSync('/var/log/syslog')) {
...
exports.RaspberryPiProvider = RaspberryPiProvider;
