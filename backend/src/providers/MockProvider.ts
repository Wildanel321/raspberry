import { 
  ...
  DisplayStatus, SecurityStatus, PackageStatus
} from './ISystemProvider';

export class MockProvider implements ISystemProvider {
  ...
  private packagesList: PackageStatus = {
    installedCount: 924,
    updatesAvailable: [
  ...
  private displayState: DisplayStatus = {
    connected: true,
  ...
  private initializeGPIOState() {
    const layout: { pin: number; gpio: number | null; name: string; type: 'power' | 'ground' | 'gpio' | 'special' }[] = [
